...
if x > 0:
  ...
  print(2*x)
      ...
      \item All other Mathematical content a student learns has a coherent and consistent framework/language from which it is built from.
  ...
  \item With the set theoretic framework, a big bulk of Preliminary coursework is building up an understanding of different types of Relations over $\mathbb{R}^2$ and Functions $f: A \subseteq \mathbb{R} \rightarrow \mathbb{R}$. For example:
    ...
    \end{enumerate}
  \item Currently, students believe that a necessary and sufficient condition for a function to have an inverse is that $f$ is one-to-one - which is in general not true. This is because the current Syllabus defaults setting all functions to be $f: A \subseteq \mathbb{R} \rightarrow f(A) \subseteq \mathbb{R}$ to avoid dealing with the co-domain concept. In the absence of the co-domain concept, an incorrect theorem such as this is being brought into universities that then need to be (sometimes painstakingly) undone\footnote{There's always that one student who asks `so the co-domain is just the range?' after an hour lecture on the subject.}. Shouldn't we make sure there is a smooth connection of exiting from high school mathematics and entering into tertiary mathematics?
  ...
  \item The concept of Sequences and Series being placed under the umbrella topic title of Financial Mathematics is bewildering. It shows a complete misplacement of importance of the concept in the Syllabus. The application of Sequences and Series to Financial Modelling is but \emph{one} use of this concept that is ubiquitous in other areas of mathematical study such as Point Set Topology.
  ...
  \item The concepts covered in the Combinatorics chapter are quite good. However, a more set theoretic focus yields many advantages such as: Permutations of a set $X$ are one-to-one correspondences (bijections) of the set $X$ onto itself, i.e. $\sigma: X \tilde{\rightarrow} X$.
